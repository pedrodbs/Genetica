...
namespace Genesis.Elements
{
	public class ExpressionConverter : IDisposable
	{
		...
		{
			// cleans expression
			if (prefixNotationExpression == null) return null;
			foreach (var superfluousStr in _parentheses)
				prefixNotationExpression = prefixNotationExpression.Replace(superfluousStr, "");
		...
		public string ToPrefixNotation(IElement element)
		{
			...
			foreach (var child in element.Children)
				strBuilder.AppendFormat("{0} ", ToPrefixNotation(child));
