	...
	public static class TreeExtensions
	{
		...
		public static uint GetMaxDepth(this IElement element)
		{
			return (element?.Children == null || element.Children.Count == 0)
				? 0 : 1 + element.Children.Max(child => child.GetMaxDepth());
		}
		...
		{
			return (element == null)
				? 0 : (element.Children == null || element.Children.Count == 0)
					? 1 : (uint)element.Children.Sum(child => child.GetMaxBreadth());
		}
		...
		/// search manner.
		/// </summary>
		/// <returns>The <see cref="IElement"/> at the given index, or <see langword="null" /> if the given index is greater 
		/// than or equal to the <see cref="IElement.Count"/>.</returns>
		...
		public static ISet<IElement> GetSubCombinations(this IElement element)
		{
			...
			var childrenSubElements = new List<IEnumerable<IElement>>();
			for (var i = 0; i < element.Children.Count; i++)
			{
				var subElems = element.Children[i].GetSubElements();
				subElems.Add(element.Children[i]);
				childrenSubElements.Add(subElems);
			}

			// creates new elements where each child is replaced by some sub-element
			var allChildrenCombs = childrenSubElements.GetAllCombinations();
			foreach (var children in allChildrenCombs)
				if (children.Count == element.Children.Count)
					combs.Add(element.CreateNew(children));

			// adds also the sub-combinations of children
			for (var i = 0; i < element.Children.Count; i++)
			{
				var subCombs = element.Children[i].GetSubCombinations();
				foreach (var subComb in subCombs)
					combs.Add(subComb);
			}

			childrenSubElements.Clear();
			return combs;
		}
		...
		{
			// checks trivials
			if (element?.Count >= other?.Count || other.Children?.Count == 0) return false;

			// search the descendants for the given element
			return other.Children.Any(child => element.Equals(child) || element.IsSubElementOf(child));
		}
		...
		{
			// checks trivials
			if (element?.Count <= other?.Count || element.Children?.Count == 0) return false;

			// search the descendants for the given element
			return element.Children.Any(child => other.Equals(child) || child.ContainsSubElement(other));
		}
	}
}
